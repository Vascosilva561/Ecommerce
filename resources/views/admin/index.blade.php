                 ...
                 <div class="row">
                     ...
                     <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12" style="margin-bottom:1px;">
                         <div class="admin-content analysis-progrebar-ctn res-mg-t-30">
                             <h4 class="text-left text-uppercase"><b>Total de vendas: </b></h4>
                             <div class="row vertical-center-box vertical-center-box-tablet">
 ...
 @section('script')
     <script>
         ...
         const now = new Date();
         for (let i = 0; i < 31; i++) {
             const valueDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() - 30 + i);
             const dateString = valueDate.toISOString().split("T")[0];
             values[dateString] = [i, 0];
         }
         console.log("values", values);
         @foreach ($per_day_sales as $sale)
             values["{{ $sale->date }}"] = [values["{{ $sale->date }}"][0], {{ $sale->total }}];
         @endforeach
         ...
         $("#curved-line-chart")[0] && $.plot($("#curved-line-chart"), [{
             ...
             if (item) {
                 const point = Math.ceil(item.datapoint[0]);

                 $(".flot-tooltip").html(dataDate[point] + ": " + data[point][1]).css({
                     top: item.pageY + 5,
                     left: item.pageX - 120
                 }).show()
